...
Whitemagic is a high-performance memory and tool infrastructure designed for AI agents. It provides 208 MCP tools (or 28 in PRAT mode), tiered memory with 5D holographic coordinates and a Galactic Map lifecycle, ethical governance, multi-galaxy project-scoped databases, Edgerunner Violet security layer, local AI agent loops (Ollama), and polyglot accelerators in 9 languages.
...
# Classic mode — all 208 individual tools
...
*   `whitemagic/`: Core Python package (~170K LOC, 208 MCP tools across 30+ domain files).
