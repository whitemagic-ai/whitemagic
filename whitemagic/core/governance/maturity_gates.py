...
def _check_radiant() -> list[GateResult]:
    ...
    # Check for cascade orchestrator
    try:
        results.append(GateResult("orchestrator", True, "ConductorOrchestrator available"))
    except Exception as exc:
        results.append(GateResult("orchestrator", False, f"Orchestrator unavailable: {exc}"))
